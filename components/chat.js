...
import initFeathers from '../lib/initFeathers';
...
class Chat extends Component {
  sendMessage(ev) {
    ...
    if(text) {
      this.client.service('messages').create({ text }).then(() => {
        input.value = '';
      });
    }
  ...
  componentDidMount() {
    this.client = initFeathers();
    this.scrollToBottom = this.scrollToBottom.bind(this);
    this.client.service('messages').on('created', this.scrollToBottom);
    this.scrollToBottom();
  }

  componentWillUnmount() {
    // Clean up listeners
    this.client.service('messages').removeListener('created', this.scrollToBottom);
  }
      ...
      <div className="flex flex-row flex-1 clear">
        <aside className="sidebar col col-3 flex flex-column flex-space-between">
          ...
          <footer className="flex flex-row flex-center">
            <a href="#" onClick={() => this.client.logout()} className="button button-primary">
              Sign Out
            </a>
          </footer>
        </aside>
